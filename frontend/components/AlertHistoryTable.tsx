...
export function AlertHistoryTable({ alerts }: { alerts: AlertsInfo[] }) {
  ...
  const paged = filtered.slice(safeePage * PAGE_SIZE, (safeePage + 1) * PAGE_SIZE)
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex flex-wrap gap-3 bg-gray-900 border border-gray-800 rounded-xl p-4">
        <div className="flex flex-col gap-1">
          ...
          <select value={filterStatus} onChange={e => { setFilterStatus(e.target.value as typeof filterStatus); setPage(0) }} className={selectCls}>
        ...
        <div className="flex flex-col gap-1">
          ...
          <select value={filterZone} onChange={e => { setFilterZone(e.target.value); setPage(0) }} className={selectCls}>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input type="date" value={filterDate} onChange={e => { setFilterDate(e.target.value); setPage(0) }} className={selectCls} />
        </div>
        {hasFilter && (
          <div className="flex flex-col gap-1 justify-end">
            <button
              onClick={() => { setFilterStatus('all'); setFilterZone('all'); setFilterDate(''); setPage(0) }}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-x-auto">
        ...
        {filtered.length > 0 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-800">
            <span className="text-sm text-gray-500">{filtered.length} alerts total</span>
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-gray-400">Page {safeePage + 1} of {totalPages}</span>
              <button
                onClick={() => setPage(p => p + 1)}
                disabled={safeePage + 1 >= totalPages}
                className="px-3 py-1 rounded-lg bg-gray-800 text-gray-300 text-sm hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Next
              </button>
