...
import { SENSORS, CHART_DATA, Alert, SEVERITY_STYLES } from '@/lib/data'
...
export function OverviewTab({ alerts, onAcknowledge, onStartResolve, onViewAllAlerts }: Props) {
            ...
            {active.slice(0, 3).map(a => (
              <div key={a.id} className="flex items-center gap-3 p-3 bg-gray-800 rounded-lg">
                ...
                <div className="flex gap-1.5 flex-shrink-0">
                  <button onClick={() => onAcknowledge(a.id)} className="text-[10px] px-2 py-1 bg-yellow-500/20 text-yellow-400 rounded hover:bg-yellow-500/30 transition-colors">ACK</button>
                  <button onClick={() => onStartResolve(a.id)} className="text-[10px] px-2 py-1 bg-green-500/20 text-green-400 rounded hover:bg-green-500/30 transition-colors">Resolve</button>
                </div>
              </div>
            ))}
          </div>
